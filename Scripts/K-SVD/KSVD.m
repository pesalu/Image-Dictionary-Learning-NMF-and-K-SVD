...
for iterNum = 1:param.numIteration
    ...
    if (iterNum>1 && param.displayProgress)
        if (param.errorFlag==0)
            ...
            output.totalerr(iterNum-1) = norm(Data-[FixedDictionaryElement,Dictionary]*CoefMatrix,'fro');

           %Compute the ssim-index between V and WH
	   ...
	   output.ssim(iterNum-1)=ssim_index(Vdum8,WHdum8);

	   printf('%d  %f  %f\n',iterNum-1,output.totalerr(iterNum-1),output.ssim(iterNum-1));
